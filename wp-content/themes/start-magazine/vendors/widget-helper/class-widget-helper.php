...
if ( ! class_exists(' Start_Magazine_Widget_Helper' ) ) {
	...
	* @since 1.0.0
	*/
	class Start_Magazine_Widget_Helper extends WP_Widget {
			...
			switch ( $type ) {
				...
				case 'image':
					...
					<div class="widget-field-image">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
						<input type="button" class="button button-primary wh-image-picker" value="<?php esc_attr_e( 'Upload', 'start-magazine' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'start-magazine' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'start-magazine' ); ?>" />
						...
						<input type="button" class="button button-secondary btn-image-remove" value="<?php esc_attr_e( 'Remove', 'start-magazine' ); ?>" <?php echo ( empty( $value ) ) ? 'style="display:none;"' : ''; ?> />
