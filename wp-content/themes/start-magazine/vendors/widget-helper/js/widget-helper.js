var start_magazine_upload_frame_obj;
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var smartMagazineImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'start-magazine-state-insert',
			...
			start_magazine_upload_frame_obj = wp.media.frames.start_magazine_upload_frame_obj = wp.media({
				button: {
					text: $( this ).data( 'uploader_button_text' )
				},
				state : 'start-magazine-state-insert',
				states : [
			...
			start_magazine_upload_frame_obj.on( 'select', function() {

				var state = start_magazine_upload_frame_obj.state('start-magazine-state-insert');
			...
			start_magazine_upload_frame_obj.open();
		});
